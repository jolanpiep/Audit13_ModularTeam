...
bool FAssetToolsPopulateClassDefaultsTest::RunTest(const FString& Parameters)
{
	...
	const USMAssetTestInstance* CDO = CastChecked<USMAssetTestInstance>(NewBP->GeneratedClass->GetDefaultObject(false));
	...
	USMAssetTestInstance* NewAssetData = NewObject<USMAssetTestInstance>();
	{
		...
		CDO = CastChecked<USMAssetTestInstance>(NewBP->GeneratedClass->GetDefaultObject(false));
...
bool FAssetToolsExportImportAssetTest::RunTest(const FString& Parameters)
{
	...
	USMInstance* OriginalCDO = CastChecked<USMInstance>(NewBP->GeneratedClass->GetDefaultObject(false));
	USMInstance* ImportedCDO = CastChecked<USMInstance>(ImportResult.Blueprint->GeneratedClass->GetDefaultObject(false));
