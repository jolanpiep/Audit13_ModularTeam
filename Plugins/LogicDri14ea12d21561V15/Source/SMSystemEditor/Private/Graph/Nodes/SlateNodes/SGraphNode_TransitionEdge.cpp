	...
	[
		...
		[
			SNew(STextBlock)
				...
				.TextStyle(FSMUnrealAppStyle::Get(), TEXT("Graph.TransitionNode.TooltipRule"))
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		.Padding(FMargin(6.f,0.0f))
		[
...
void SGraphNode_TransitionEdge::UpdateGraphNode()
{
	...
	[
		...
		[
			...
			.AutoHeight()
			.HAlign(HAlign_Center)
			.Padding(TAttribute<FMargin>::Create(TAttribute<FMargin>::FGetter::CreateSP(this, &SGraphNode_TransitionEdge::GetNameMargin)))
			[
				SNew(SBorder)
				.BorderImage(FSMUnrealAppStyle::Get().GetBrush("Graph.StateNode.ColorSpill"))
				.Visibility(this, &SGraphNode_TransitionEdge::GetTransitionNameVisibility)
				.OnMouseButtonDown(this, &SGraphNode_TransitionEdge::OnTransitionNameMouseDown)
				.Padding(GetNameBorderPadding())
				[
					SNew(SHorizontalBox)
					+ SHorizontalBox::Slot()
					.AutoWidth()
					[
						...
						[
							...
							.Font(FSMUnrealAppStyle::GetFontStyle("NormalFontBold"))
							// Similar to Graph.StateNode.NodeTitle settings
							.ColorAndOpacity(FLinearColor(230.0f/255.0f,230.0f/255.0f,230.0f/255.0f))
							.ShadowColorAndOpacity(FLinearColor(0.f,0.f,0.f, 0.7f))
							.ShadowOffset(FVector2D::UnitVector)
							.Text(NodeTitle.Get(), &SNodeTitle::GetHeadTitle)
							.OnVerifyTextChanged(this, &SGraphNode_TransitionEdge::OnVerifyNameTextChanged)
							.OnTextCommitted(this, &SGraphNode_TransitionEdge::OnNameTextCommited)
							.IsReadOnly(true)
							.IsSelected(this, &SGraphNode_TransitionEdge::IsSelectedExclusively)
						]
						+ SOverlay::Slot()
						[
					...
					[
						SNew(STextBlock)
						.Margin(FMargin(0.f, -2.5f, 0.f, 0.f))
						.TextStyle(FSMUnrealAppStyle::Get(), "RichTextBlock.Bold")
						.Visibility(this, &SGraphNode_TransitionEdge::GetTransitionNameWarningVisibility)
						.Text(LOCTEXT("TransitionNameWarningIndicatorText", "*"))
					]
				]
			]
			+ SVerticalBox::Slot()
...
TSharedPtr<SToolTip> SGraphNode_TransitionEdge::GetComplexTooltip()
{
	...
	Widget->AddSlot()
		...
		[
			...
			+ SVerticalBox::Slot()
			// The full state to state name, only displayed if the user changed the transition name.
			.AutoHeight()
			...
			[
				...
				.Visibility_Lambda([TransitionNode]()
				{
					return TransitionNode && TransitionNode->IsUsingCustomName() ? EVisibility::Visible : EVisibility::Collapsed;
				})
				.TextStyle(FSMEditorStyle::Get(), "SMGraph.Tooltip.Info")
				.Text(FText::Format(LOCTEXT("TransitionPopupDefaultTitle", "{0}"), FText::FromString(TransitionNode->GetDefaultTransitionName())))
			]
			+ SVerticalBox::Slot()
			// A warning in case the transition name might change.
			.AutoHeight()
			.Padding(2.f, 0.f, 0.f, 0.f)
			[
				SNew(STextBlock)
				...
				{
					return CanTransitionNameChange() ? EVisibility::Visible : EVisibility::Collapsed;
				})
				.TextStyle(FSMUnrealAppStyle::Get(), "SmallText")
				.Text(LOCTEXT("TransitionNameWarningDescription", "* This transition name may change at run-time"))
			]
		];
...
EVisibility SGraphNode_TransitionEdge::GetTransitionNameVisibility() const
{
	EVisibility NameVisibility = EVisibility::Collapsed;
	if (const USMGraphNode_TransitionEdge* TransitionNode = GetTransition())
	{
		const USMEditorSettings* EditorSettings = FSMBlueprintEditorUtils::GetEditorSettings();

		switch (TransitionNode->IsUsingCustomName() ?
			EditorSettings->DisplayCustomTransitionName : EditorSettings->DisplayDefaultTransitionName)
		{
		case ESMNodeInfoVisibility::Never:
			{
				NameVisibility = EVisibility::Collapsed;
				break;
			}
		case ESMNodeInfoVisibility::Always:
			{
				NameVisibility = EVisibility::Visible;
				break;
			}
		case ESMNodeInfoVisibility::OnHover:
			{
				NameVisibility = (TransitionNode->IsHovered() || FSMBlueprintEditorUtils::IsNodeSelected(GraphNode)) ?
	...
	CachedNameVisibility = NameVisibility;
	
	return NameVisibility;
}

EVisibility SGraphNode_TransitionEdge::GetTransitionNameWarningVisibility() const
{
	if (CachedNameVisibility != EVisibility::Collapsed && CachedNameVisibility != EVisibility::Hidden)
	{
		if (CanTransitionNameChange())
		{
			return EVisibility::Visible;
		}
	}
...
FReply SGraphNode_TransitionEdge::OnTransitionNameMouseDown(const FGeometry& MyGeometry,
	const FPointerEvent& MouseEvent)
{
	if (GraphNode && OwnerGraphPanelPtr.IsValid())
	{
		// Select the node in the editor -- Engine selection is avoided due to our custom name margin.
		
		if (MouseEvent.IsShiftDown() || MouseEvent.IsControlDown())
		{
			OwnerGraphPanelPtr.Pin()->SelectionManager.SetNodeSelection(GraphNode, true);
		}
		else
		{
			OwnerGraphPanelPtr.Pin()->SelectionManager.SelectSingleNode(GraphNode);
		}
...
bool SGraphNode_TransitionEdge::CanTransitionNameChange() const
{
	if (const USMGraphNode_TransitionEdge* Transition = GetTransition())
	{
		if (Transition->IsFromAnyState() && !Transition->IsUsingCustomName())
		{
			return true;
		}
	}
...
FMargin SGraphNode_TransitionEdge::GetNameMargin() const
{
	// Remove extra width and height of the name widget so as not to make the selection overly large, which can
	// cause both very wide margins on reroute nodes, as well as inconsistencies when only some nodes display their name.
	...
	const FVector2D TextSize = InlineEditableText->GetDesiredSize();
	if (!TextSize.IsZero())
	{
		// TextSize goes to zero and back every other frame on interactive changes which can be jarring
		CachedNameSize = TextSize;
	}

	const float NameBorderVerticalPadding = GetNameBorderPadding() * 2.f;
	const float Width = CachedNameSize.X / 2.f;
	const float Height = CachedNameSize.Y + NameBorderVerticalPadding;
	return FMargin(
		-Width,
		CachedNameVisibility == EVisibility::Collapsed ? NameBorderVerticalPadding : -Height,
		-Width,
		0.f);
}
