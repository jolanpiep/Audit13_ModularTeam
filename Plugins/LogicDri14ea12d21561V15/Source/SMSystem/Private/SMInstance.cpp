...
void USMInstance::FInitializeInstanceAsyncTask::DoWork()
{
	if (Instance.IsValid())
	{
		while (IsGarbageCollectingAndLockingUObjectHashTables())
		{
			LD_LOG_INFO(TEXT("Initialize async task attempting to run while garbage collection is occurring, waiting..."))
			checkf(!IsInGameThread(), TEXT("InitializeAsync is running on the game thread during garbage collection, no way to safely continue. Does the system support multi-threading?"));
...
{
	if (!IsValid(this)
		|| HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed) // UE 5.5 requires this to be checked early.
		|| IsUnreachable()
		|| (!IsInitialized() && !bTickBeforeInitialize)
		|| !CanEverTick()
		|| IsTemplate()) // Intermittently IsTemplate may fail in this scenario so it should be checked last.
	{
		return false;
	}
...
ETickableTickType USMInstance::GetTickableTickType() const
{
	...
	if (!IsValid(this)
		|| !IsTickRegistered()
		|| IsTemplate()
		|| (GetComponentOwner() // We only want the component to tick us if present.
			&& !GetComponentOwner()->bLetInstanceManageTick))
	{
...
void USMInstance::Serialize(FArchive& Ar)
{
...
void USMInstance::Initialize(UObject* Context)
{
	...
	{
		// If this blueprint isn't compiled and it contains a call to a parent there's a good chance the class
		// data is stale. When exposed functions execute UE may crash. GitHub #200.
		if (Blueprint->IsPossiblyDirty())
		{
			if (const USMBlueprintGeneratedClass* GeneratedClass = Cast<USMBlueprintGeneratedClass>(Blueprint->GeneratedClass))
			{
				if (GeneratedClass->bHasStaleParentData)
				{
					LD_LOG_ERROR(TEXT("Cannot initialize state machine blueprint %s. It needs to be recompiled."), *GetName());
					return;
				}
			}
		}
	...
	// Cache graph property data used during node creation.
	CreateCachedPropertyData();
	...
	RootStateMachine.SetNodeInstanceClass(GetRootStateMachineNodeClass());
	...
	if (bIsPrimaryReferenceOwner)
	{
		...
		if (IsInitializingAsync())
		{
			struct FLocal
			{
				static void OnDispatchGameThread(const TWeakObjectPtr<USMInstance> InInstance)
				{
					...
					// Instance could have canceled async initialization such as from shutting down.
					if (InInstance.IsValid() && !InInstance->IsInitialized() && InInstance->IsInitializingAsync())
					{
						// SkinnedAssetCompilingManager could potentially trigger this OnPostLoad. This is rare and
						// should only ever occur in the editor.
						const bool bIsPostLoad = FUObjectThreadContext::Get().IsRoutingPostLoad;

						// GC might be true if post load caused us to defer to the next frame?
						const bool bIsGC = IsGarbageCollectingAndLockingUObjectHashTables();
						if (bIsPostLoad || bIsGC)
						{
							if (const UWorld* World = InInstance->GetWorld())
							{
								LD_LOG_INFO(TEXT("Attempted to FinishInitialize async during either PostLoad or GarbageCollection. Deferring until next tick. PostLoad: %d, GC: %d."),
									bIsPostLoad, bIsGC)
								World->GetTimerManager().SetTimerForNextTick(FTimerDelegate::CreateStatic(&FLocal::OnDispatchGameThread, InInstance));
							}
							else
							{
								LD_LOG_ERROR(TEXT("Attempted to FinishInitialize async during either PostLoad or GarbageCollection. Cannot defer to next tick - World is null. PostLoad: %d, GC: %d."),
									bIsPostLoad, bIsGC)
							}
						...
						InInstance->FinishInitialize();
						InInstance->CleanupAsyncInitializationTask();
					}
				}
			};
			
			FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
				FSimpleDelegateGraphTask::FDelegate::CreateStatic(&FLocal::OnDispatchGameThread, MakeWeakObjectPtr(this)),
			TStatId(),
			nullptr,
...
void USMInstance::Start()
{
	...
	if (!IsValid(this) || HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed) || IsUnreachable())
	{
		return;
	}
...
void USMInstance::Shutdown()
{
	...
	const bool bContextDestroyed = !IsValid(Context)
		|| Context->HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed)
	...
	const bool bIsBeingDestroyed = bContextDestroyed || !IsValid(this) ||
		HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed) || IsUnreachable();
...
void USMInstance::ActivateStateLocally(const FGuid& StateGuid, bool bActive, bool bSetAllParents, bool bActivateNow)
{
	...
	// Build out a list of all states that need activation. This is done to create a proper order of FSM -> sub state
	// activation, and is especially important for references so they can start through the reference and fire appropriate
	// events.
	TArray<FSMState_Base*> StatesToActivate;
	{
		FSMState_Base* State = GetStateByGuid(StateGuid);
		while (State)
		{
			StatesToActivate.Insert(State, 0);
			if (!bSetAllParents || State->GetOwnerNode() == nullptr)
			{
				break;
			}
			
			FSMStateMachine* StateMachineOwner = static_cast<FSMStateMachine*>(State->GetOwnerNode());
			State = (bActive || /* Deactivating */ !StateMachineOwner->HasActiveStates()) ? StateMachineOwner : nullptr;
		}
	}
	
	for (FSMState_Base* State : StatesToActivate)
	{
...
void USMInstance::FinishInitialize()
{
	...
	const bool bFinishedAsync = bInitializingAsync;
	if (bFinishedAsync)
	{
		if (IsPrimaryReferenceOwner())
		{
	...
	// Not needed after initialize.
	CachedPropertyData.Reset();

	if (bFinishedAsync)
	{
		// Fire last so the caller can continue execution after all other initialization completes.
...
void USMInstance::InitializeAsync(UObject* Context, const FOnStateMachineInstanceInitializedAsync& OnCompletedDelegate)
{
	...
	AsyncInitializationTask = MakeUnique<FAsyncTask<FInitializeInstanceAsyncTask>>(this, Context);
	if (IsInGameThread())
	{
...
{
	return RootStateMachine.IsStateMachineInEndState();
}
...
void USMInstance::NotifyTransitionTaken(const FSMTransition& Transition)
{
	...
	{
		const FString ContextName = GetContext() ? GetContext()->GetName() : TEXT("NoContext");
		LD_LOG_INFO(TEXT("[%s-%s] Transition taken: %s"), *ContextName, *GetName(), *TransitionInfo.ToString());
	}
#endif
	...
	if (ReferenceOwner)
	{
		ReferenceOwner->NotifyTransitionTaken(Transition);
	}
}
...
void USMInstance::NotifyStateChange(FSMState_Base* ToState, FSMState_Base* FromState)
{
	...
	{
		const FString ContextName = GetContext() ? GetContext()->GetName() : TEXT("NoContext");
		LD_LOG_INFO(TEXT("[%s-%s] State change: from %s to %s"), *ContextName, *GetName(), *FromStateInfo.ToString(), *ToStateInfo.ToString());
	}
#endif
	...
	if (ReferenceOwner)
	{
		ReferenceOwner->NotifyStateChange(ToState, FromState);
	}
}
...
{
	return HasAnyFlags(RF_ClassDefaultObject) ? RootPathGuidCache : CastChecked<USMInstance>(GetClass()->GetDefaultObject())->RootPathGuidCache;
}
...
void USMInstance::SetRootPathGuidCache(const TMap<FGuid, FSMGuidMap>& InGuidCache)
{
	...
	{
		CastChecked<USMInstance>(GetClass()->GetDefaultObject())->RootPathGuidCache = InGuidCache;
	}
}

TSharedPtr<FSMCachedPropertyData, ESPMode::ThreadSafe> USMInstance::GetCachedPropertyData()
{
	if (!CachedPropertyData.IsValid())
	{
		CreateCachedPropertyData();
	}
...
const TMap<FName, TObjectPtr<UObject>>& USMInstance::GetGeneratedSubObjectTemplates() const
{
	const USMBlueprintGeneratedClass* BPGC = CastChecked<USMBlueprintGeneratedClass>(GetClass());
	return BPGC->GetSubObjectTemplates();
}

void USMInstance::CreateCachedPropertyData()
{
